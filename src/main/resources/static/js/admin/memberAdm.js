...
document.addEventListener("DOMContentLoaded", function () {
    const itemsPerPage = 15; // 한 페이지당 15개씩
    let currentPage = 1; // 기본 페이지
    let totalItems = document.querySelectorAll(".APaboard tbody tr").length; // 전체 데이터 개수
    let totalPages = Math.ceil(totalItems / itemsPerPage); // 전체 페이지 수

    function showPage(page) {
        let rows = document.querySelectorAll(".APaboard tbody tr");
